...
public:
	...
	static const void GetAction_Mouse(const SDL_Event& user_input, const CONTROLLER_STATE& state ) {
		if (state == IN_GAME) {
		...
		if (state == MENU) {
			if (SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT)) {
				SDL_Log("Mouse Button 1 (left) is pressed.");
				if (CollisionManager::mouse_button_collision) {
					Game::Select();
				}
				else
					return;
	...
	static const void HandleUserInput_Mouse(const SDL_Event& user_input, const CONTROLLER_STATE& state) 
	{
		if (state == INACTIVE)
			return;
		else
			GetAction_Mouse(user_input, state);
	}

	// Same as mouse but with the keyboard
	static const void GetAction_Keyboard(const SDL_Event& user_input,const CONTROLLER_STATE& state )
	{
		if (state == IN_GAME) {
	...
	// Same as mouse but with the keyboard
	static const void HandleUserInput_Keyboard(const SDL_Event& user_input, const CONTROLLER_STATE state)
	{
		if (state == INACTIVE)
			return;
		else
