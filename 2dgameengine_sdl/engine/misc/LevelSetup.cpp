...
#include "./Entity/Component/TextLabelComponent.h"
#include "./Entity/Component/ProjectileEmitterComponent.h"
#include "./Entity/Component/HealthComponent.h"
#include "./Entity/Component/ButtonComponent.h"

#include <cmath>

void LoadLevelAssets(sol::table levelAssets)
{
    ...
    while (true) {
        ...
        if (existsAssetIndexNode == sol::nullopt) {
            ...
            if (assetType.compare("texture") == 0) {
                ...
                Game::asset_manager->AddTexture(assetId, assetFile.c_str());
            }
            if (assetType.compare("font") == 0) {
                std::string assetId = asset["id"];
                std::string assetFile = asset["file"];
                int fontSize = asset["fontSize"];
                Game::asset_manager->AddFont(assetId, assetFile.c_str(), fontSize);
            }
        }
        assetIndex++;
    }
...
void LoadLevelEntities(sol::table levelEntities)
{
    ...
    while (true) {
        ...
        if (existsEntityIndexNode == sol::nullopt) {
            ...
            // Add new entity
            Entity& newEntity(Game::entity_manager->AddEntity(entityName, entityLayerType));
            std::cout << &newEntity << " ...\n";
            ...
            if (existsProjectileEmitterComponent != sol::nullopt) {
                ...
                Entity& projectile(Game::entity_manager->AddEntity("projectile", PROJECTILE_LAYER));
            ...
            if (existsButtonComponent != sol::nullopt) {
                ...
                Game::entity_manager->buttons.AddButton(&newEntity);
            }
        }
        entityIndex++;
    }
...
void LevelSetup::LoadLevel(Levels levelNumber, Map* map)
{
    // We set the current level
    // First we clear the manager if its not empty
    if (!Game::entity_manager->HasNoEntities()) {
        Game::entity_manager->DeleteData();
    }
    if (!Game::asset_manager->HasNoAssets()) {
        Game::asset_manager->DeleteData();
    }
...
void LevelSetup::LoadPresets(Levels levelNumber) {
    ...
    while (true) {
        ...
        if (existsEntityIndexNode == sol::nullopt) {
            ...
            // Add new entity
            Preset& newPreset(Game::entity_manager->presets.AddPreset(entityName, entityLayerType));
